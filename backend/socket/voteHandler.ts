...
export function handleVote(io: Server, socket: Socket) {
    ...
    socket.on('SUBMIT_VOTE', async (itemIds: number[]) => {
        ...
        // 입력 데이터 검증 및 정규화
        console.log('받은 itemIds:', itemIds, 'type:', typeof itemIds);

        let normalizedItemIds: number[];

        if (!itemIds) {
            console.warn('itemIds가 null 또는 undefined입니다.');
            return;
        }

        if (Array.isArray(itemIds)) {
            normalizedItemIds = itemIds;
        } else if (typeof itemIds === 'number') {
            // 단일 숫자인 경우 배열로 변환
            normalizedItemIds = [itemIds];
        } else if (typeof itemIds === 'string') {
            // 문자열인 경우 숫자로 변환 후 배열로 처리
            const parsed = parseInt(itemIds, 10);
            if (isNaN(parsed)) {
                console.warn('유효하지 않은 itemId 문자열:', itemIds);
                return;
            }
            normalizedItemIds = [parsed];
        } else {
            console.warn('유효하지 않은 itemIds 타입:', typeof itemIds, itemIds);
            return;
        }
        ...
        console.log('투표 참여:', normalizedItemIds);

        const prevVotes = userVotes.get(userId) ?? new Set<number>();
        const nextVotes = new Set<number>();
        let hasVoteChanged = false;

        const validItemIds = normalizedItemIds.filter(itemId => validateItem(currentVote!, itemId, userId));
        ...
        if(currentVote.isMultiple){ // 중복 투표 모드
            ...
            const newItemId = normalizedItemIds[0];
