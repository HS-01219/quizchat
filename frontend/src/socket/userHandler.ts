...
export const useUserHandler = () => {
  ...
  // 위에 주석처리 된 부분이 오류가 나서 실행이 안되길래 아래처럼 userJoined, userLeaved 추가해놨습니다.
  const userJoined = (data: {
    ...
    console.log(
      `${data.nickName} 님이 방에 참여했습니다. userId : ${data.userId}`
    );
    updateUserCnt(data);
  };

  const userLeaved = (data: {
    currentUsers: number;
    userId: number;
    nickName: string;
  }) => {
  ...
  const updateUserCnt = (data: {
    currentUsers: number;
    userId: number;
    nickName: string;
  }) => {
    console.log(
      `${data.nickName} 님이 방에 참여했습니다. 현재 인원: ${data.currentUsers}`
    );
  ...
  const joinRoom = (data: {
    ...
    console.log(
      `현재 퀴즈 상태 : ${data.roomState.quizState.isActive} 현재 투표 상태 : ${data.roomState.voteState.isActive}`
    );
